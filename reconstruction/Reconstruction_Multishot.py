...
def filtercircle(inputpic,tag):
    ...
    for i in range(0, nrows):         
      for j in range(0, ncols):
           ...
           if distance < 0.85*r and distance>0.10*r: continue #0.5 is good
...
def makegraph(inputpic):
    ...
    gr = ROOT.TGraph( len(pixels_xy_rot), x, y )
    return gr
...
def findintersection(fit1,fit2,low,high):
    ...
    points = [ 0 + k*0.00001 for k in range(0,steps+1)]
    for k in points:
         k1 = fit1.Eval(k)
         k2 = fit2.Eval(k)
         if abs(k2-k1)<max_dk:
...
def makefits(inputh,inputv,tag):
    ...
    gr1.Fit("pol1","RQ","",minx_1-5,maxx_1+5);
    gr2.Fit("pol1","RQ","",minx_2-5,maxx_2+5);
    ...
    histoframe = ROOT.TH2F("","",ncols*10,-ncols,ncols,nrows*10,0,1.5*nrows)
    ...
    histoframe.GetXaxis().SetTitle("X rotated pixel coordinate")
    histoframe.GetYaxis().SetTitle("Y rotated pixel coordinate")
    histoframe.Draw()
...
def drawresults(inputpic,p,fit1,fit2,tag):
    ...
    for i in range(0, nrows):         
      for j in range(0, ncols):         
           if math.sqrt(  (j-p[0])*(j-p[0]) + (i-p[1])*(i-p[1]) ) < 4:
               img[i,j,0]=0
               img[i,j,1]=0               
               img[i,j,2]=255            
...
img_original_1   = cv2.imread('examples/point_1/contour_1.jpg')
img_original_2   = cv2.imread('examples/point_1/contour_2.jpg')
img_original_3   = cv2.imread('examples/point_1/contour_3.jpg')
img_original_4   = cv2.imread('examples/point_1/contour_4.jpg')
img_shot_1       = cv2.imread('examples/point_1/shot_1.jpg')
img_shot_2       = cv2.imread('examples/point_1/shot_2.jpg')
img_shot_3       = cv2.imread('examples/point_1/shot_3.jpg')
img_shot_4       = cv2.imread('examples/point_1/shot_4.jpg')
...
img_horizontal_1,img_vertical_1 = filterhorizontalandvertical(img_nocircle_1,15,"1")
img_horizontal_2,img_vertical_2 = filterhorizontalandvertical(img_nocircle_2,15,"2")
img_horizontal_3,img_vertical_3 = filterhorizontalandvertical(img_nocircle_3,15,"3")
img_horizontal_4,img_vertical_4 = filterhorizontalandvertical(img_nocircle_4,15,"4")
...
#Noozle position in four corner shots (below)
n_1 = [-4.563859498681640, 95.56437424628370]
n_2 = [28.436140501318400, 95.56437424628370] 
n_3 = [28.436140501318400, 129.56437424628400]
n_4 = [-4.563859498681640, 129.56437424628400]
...
print "    - Top      (33.68 ): %.3f"%top
print "    - Bottom   (32.93 ): %.3f"%bot
print "    - Left     (33.682): %.3f"%lef
print "    - Right    (33.682): %.3f"%rig
